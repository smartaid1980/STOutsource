DROP TABLE IF EXISTS `a_feedback_supplier`;
CREATE TABLE `a_feedback_supplier` (
...
DROP TABLE IF EXISTS `a_feedback_po_list`;
CREATE TABLE `a_feedback_po_list` (
  ...
  KEY `FK_a_feedback_po_list_sup_id` (`sup_id`),
  CONSTRAINT `FK_a_feedback_po_list_sup_id` FOREIGN KEY (`sup_id`) REFERENCES `a_feedback_supplier` (`sup_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='採購單明細檔';

DROP TABLE IF EXISTS `a_feedback_demand_list`;
CREATE TABLE `a_feedback_demand_list` (
  ...
  KEY `FK_a_feedback_demand_list_sup_id` (`sup_id`),
  CONSTRAINT `FK_a_feedback_demand_list_sup_id` FOREIGN KEY (`sup_id`) REFERENCES `a_feedback_supplier` (`sup_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='需求單';

DROP TABLE IF EXISTS `a_feedback_leadtime_chg_log`;
CREATE TABLE `a_feedback_leadtime_chg_log` (
  `form_id` varchar(40) NOT NULL DEFAULT '' COMMENT '需求單號',
  `seq_no` varchar(5) NOT NULL DEFAULT '0001' COMMENT '序號',
  `sup_id` varchar(10) NOT NULL DEFAULT '' COMMENT '廠商代碼',
  ...
  KEY `FK_a_feedback_leadtime_chg_log_form_id_seq_no` (`form_id`,`seq_no`),
  KEY `FK_a_feedback_leadtime_chg_log_sup_id` (`sup_id`),
  CONSTRAINT `FK_a_feedback_leadtime_chg_log_form_id_seq_no` FOREIGN KEY (`form_id`,`seq_no`) REFERENCES `a_feedback_demand_list` (`form_id`,`seq_no`),
  CONSTRAINT `FK_a_feedback_leadtime_chg_log_sup_id` FOREIGN KEY (`sup_id`) REFERENCES `a_feedback_supplier` (`sup_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='交期異動紀錄';

DROP TABLE IF EXISTS `a_feedback_questions`;
CREATE TABLE `a_feedback_questions` (
  ...
  PRIMARY KEY (`qu_id`),
  KEY `FK_a_feedback_questions_form_id_seq_no` (`form_id`,`seq_no`),
  KEY `FK_a_feedback_questions_sup_id` (`sup_id`),
  CONSTRAINT `FK_a_feedback_questions_form_id_seq_no` FOREIGN KEY (`form_id`,`seq_no`) REFERENCES `a_feedback_demand_list` (`form_id`,`seq_no`),
  CONSTRAINT `FK_a_feedback_questions_sup_id` FOREIGN KEY (`sup_id`) REFERENCES `a_feedback_supplier` (`sup_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='加工問題';

DROP TABLE IF EXISTS `a_feedback_questions_reply`;
CREATE TABLE `a_feedback_questions_reply` (
  ...
  KEY `a_feedback_questions_reply_qu_id` (`qu_id`),
  CONSTRAINT `a_feedback_questions_reply_qu_id` FOREIGN KEY (`qu_id`) REFERENCES `a_feedback_questions` (`qu_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='問題回覆紀錄';

DROP TABLE IF EXISTS `a_feedback_questions_process_log`;
CREATE TABLE `a_feedback_questions_process_log` (
  ...
  PRIMARY KEY (`log_id`),
  KEY `a_feedback_questions_process_log_qu_id` (`qu_id`),
  CONSTRAINT `a_feedback_questions_process_log_qu_id` FOREIGN KEY (`qu_id`) REFERENCES `a_feedback_questions` (`qu_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='問題處理紀錄';

DROP TABLE IF EXISTS `a_feedback_questions_file`;
CREATE TABLE `a_feedback_questions_file` (
  ...
  PRIMARY KEY (`file_id`),
  KEY `a_feedback_questions_file_qu_id` (`qu_id`),
  CONSTRAINT `a_feedback_questions_file_qu_id` FOREIGN KEY (`qu_id`) REFERENCES `a_feedback_questions` (`qu_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='圖檔附件';

DROP TABLE IF EXISTS `a_feedback_questions_download_log`;
CREATE TABLE `a_feedback_questions_download_log` (
  `file_id` varchar(10) NOT NULL DEFAULT '' COMMENT '附件ID',
  ...
  KEY `a_feedback_questions_download_log_file_id` (`file_id`),
  CONSTRAINT `a_feedback_questions_download_log_file_id` FOREIGN KEY (`file_id`) REFERENCES `a_feedback_questions_file` (`file_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='附檔下載紀錄';

create view a_feedback_view_demand_questions
as 
select demand.*, 
question.class, 
question.status,
