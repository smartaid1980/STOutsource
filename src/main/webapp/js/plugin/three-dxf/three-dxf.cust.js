...
(function (ThreeDxf) {
  ...
  /**
   * In order to avoid screen freezing while drawing picture through three.js, especailly on adding TEXT scenes.
   * The scene-adding process would pause 25ms every 50ms of excuting, let the UI be responsive during the time calculating scenes.
   * In addition, I add a new parameter $progressBar to the Viewer method, in order to render the progress bar dynamically.
   * Revised by Mike. 2019/01/09
   */
  ThreeDxf.Viewer = function (data, parent, width, height, font, $progressBar) {
    var $parent = $(parent);
    ...
    (function timedChunk (data, process, context, callback) {
      let todo = data.entities.concat(),
        totalCount = todo.length,
        todoChunk = Math.floor(totalCount / 20),
        chunkCount = 0,
        progress = 0,
        isProgressBar = $progressBar && $progressBar.hasClass('progress-bar');
      ...
      function chunk () {
        ...
        do {
          process(todo.shift(), data);
          if (++chunkCount >= todoChunk) {
            progress++;
            isProgressBar && $progressBar.width(progress * 5 + '%');
            chunkCount = 0;
          }
          if (progress >= 20) isProgressBar && setTimeout(() => $progressBar.parent().addClass('hide'), 1000);
        } while (todo.length > 0 && (new Date - timeStart < 50));
      ...
      if(entity.type === 'DIMENSION') {
        if(entity.block) {
          ...
          for(var j = 0; j < block.entities.length; j++) {
            ...
            // Revised by Mike. Block entity should also be added to the scene.
            if (obj) {
      ...
      let canvasWidth = width || $parent.innerWidth();
      let canvasHeight = height || $parent.innerHeight();
      ...
      this.resize = function (canvasWidth, canvasHeight) {
        ...
        var hscale = canvasWidth / originalWidth;
        var vscale = canvasHeight / originalHeight;
