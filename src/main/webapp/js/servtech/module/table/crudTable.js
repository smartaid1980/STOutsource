...
function crudtable(config) {
  ...
  const getAllSelectedRowBling = function () {
        ...
        ajax(
          ...
          },
          {
            success: function (data) {
              ...
              $(newRow).data(
                'rowData',
                Object.assign($(newRow).data('rowData'), formData)
              )
              bling(4, 300, $(newRow).find('td'), 'rgba(0, 255, 0, 0.2)')
  ...
  const getBlingRow = function (colNum, data) {
    ...
    for (let row = 0; row < table.data().length; row++) {
      let text
      if ($(table.rows(row).nodes()[0]).find('td:eq(0)').hasClass('hidden-xs'))
    ...
    _.extend(
      {
        ...
        headerCallback: function () {
          if (
            $stkCrudTableEle.find('i.fa.fa-question-circle.fa-question-circle')
              .length === 0
          ) {
            $stkCrudTableEle
        ...
        rowCallback: function (nRow, data) {
          ...
          if (config['delete']['unavailable']) {
            if (!$nRow.children(':first').hasClass('crud-delete-unavailable')) {
          ...
          if (config['update']['unavailable']) {
            if (!$nRow.children(':last').hasClass('crud-update-unavailable')) {
    ...
    .on('click', '.stk-edit-btn', function () {
      ...
      for (var i = 0; i <= columnNames.length; i++) {
        ...
        if (config.update.start && config.update.start[i]) {
          config.update.start[i](oldTr.children[i], newTd, oldTr, newTr, table)
        } else {
  ...
  const refreshTable = () => {
    if (config.ajax) {
      ...
      if (config.read.dataset) {
        ...
        $datasetSelect = $stkCrudTableEle.prev().find('.stk-dt-dataset-select')
      ...
      return new Promise((res) => {
          ...
          newData = _.map(data, function (ele) {
            return _.map(columnNames, function (columnName, i) {
              ...
              if (config.read.end && config.read.end[i + 1]) {
                return valueToView(config.read.end[i + 1](ele[columnName], ele))
              } else if (
                $(trEleTemplate)
        ...
        // read 完之後執行
        if (config.read.finalDo && typeof config.read.finalDo === 'function') {
          config.read.finalDo()
        }
      })
    }
  ...
  return {
    ...
    refreshTable,
    getBlingRow,
    getAllSelectedRowBling, //未勾選任何東西 就閃爍警示
  }
}
