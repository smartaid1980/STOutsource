...
function drawWidget(config, tempalteEngine, allConfig) {
  ...
  if (config.chartId) {
    ...
    widgetCode.push(
      '<header>\n<span class="widget-icon"> ' + config.widgetIcon + ' </span>\n'
    )
    widgetCode.push('<h2>' + config.widgetTitle + '</h2>\n</header>\n')
  ...
  if (config.modalForm) {
    ...
    _.each(config.component, function (elem) {
      ...
      if (elem.type === 'text') {
        ...
        modalCode.push.apply(modalCode, drawStartEndDate(elem, tempalteEngine))
      } else if (elem.type === 'select') {
...
function drawStartEndDate(config, tempalteEngine) {
  ...
  if (config.formEndId) {
    ...
    if (config.formEndName) {
      tempalteEngine.toHtmlgogo.push(
        "$('[name=" + config.formEndName + "]'), " + config.formBothToday + ', '
      )
    } else {
      startEndDateCode.push('name="' + config.element + 'end" ')
      tempalteEngine.toHtmlgogo.push(
        "$('[name=" + config.element + "end]'), " + config.formBothToday + ', '
      )
    }
  }
  if (config.formEndName) {
  ...
  if (config[source + 'SelectBy'] === 'DB') {
    ...
    if (!tableExist) {
      tempalteEngine.toHtmlpreCondition.push(
        'get_' + config[source + 'SelectDBTable'] + '_list: function (done) {\n'
      )
      ...
      if (config.bindingSelectColumn) {
        ...
        tempalteEngine.toHtmlgogo.push('context.get' + elementName + 'Data()\n')
        if (tempalteEngine.toHtmlutil.length) {
          tempalteEngine.toHtmlutil.push(',')
        }
        tempalteEngine.toHtmlutil.push('\n')
        ...
        tempalteEngine.toHtmlutil.push('    ctx.' + elementName + 'Data = {}\n')
        tempalteEngine.toHtmlutil.push(
        ...
        if (config.type === 'multipleSelect') {
          ...
          tempalteEngine.toHtmlgogo.push('  _.each($(this).val(), (ele) => {\n')
          tempalteEngine.toHtmlgogo.push(
            '    _.each(context.' +
              elementName +
              'Data [ele], (value, key) => {\n'
        ...
        if (config[source + 'Id']) {
          tempalteEngine.toHtmlgogo.push("$('#" + config[source + 'Id'] + "')")
        } else {
          if (config.formName) {
            tempalteEngine.toHtmlgogo.push(
    ...
    if (isFormDataValueNotAllEmpty) {
      ...
      if (config.formId) {
        ...
        if (config.formName) {
          tempalteEngine.toHtmlgogo.push("$('[name=" + config.formName + "]')")
        } else {
          tempalteEngine.toHtmlgogo.push("$('[name=" + config.element + "]')")
        }
      }
...
function drawTable(config, tempalteEngine, allConfig) {
  ...
  _.each(config.tableCustomerBtn, function (elem) {
    ...
    if (elem.tableBtnBinding === 'Modal' && elem.tableBtnBindingModal) {
      html.push(
        ' data-toggle="modal" data-target="#' + elem.tableBtnBindingModal + '"'
      )
    ...
    if (config.tableDataFromTableModel) {
      ...
      dataShowList = _.map(config.columnList, (col) => {
        if (
          ...
          var first = true
          _.each(config.columnMap[col].tableColumnTextSelectElement, (text) => {
            if (first) {
              first = false
            } else {
              tempalteEngine.toHtmlutil.push(',\n')
        ...
        if (
          ...
          if (config.columnMap[col].tableColumnTitle) {
            html.push(' title="' + config.columnMap[col].tableColumnTitle + '"')
          }
          if (config.columnMap[col].tableColumnBtnBinding === 'Modal') {
            html.push(
              ' data-toggle="modal" data-target="#' +
      ...
      if (
        ...
        if (data.type === 'startEndDate') {
          ...
          if (data.formId) {
            ...
            tempalteEngine.toHtmlgogo.push("  $('[name=" + data.formName + ']')
          } else {
...
function drawChart(config, tempalteEngine) {
  ...
  tempalteEngine.toHtmlutil.push("      tickColor: 'rgba(186,186,186,0.2)',\n")
  tempalteEngine.toHtmlutil.push('      axisLabelFontFamily: servkit.fonts,\n')
        ...
        if (frameType === 'widget') {
          ...
          _.each(allData, (data) => {
            if (
              ...
              if (data.type === 'multipleSelect' || data.type === 'checkbox') {
                criteriaData.push(
                  "  criteria += '" + data.formBindingTableColumn + ' in("\' + '
                )
              } else if (data.type === 'startEndDate' || data.type === 'date') {
                criteriaData.push(
                  "  criteria += '" +
                    data.formBindingTableColumn +
                    ' BETWEEN "\' + '
              ...
              if (
                ...
                if (data.type === 'startEndDate') {
                  if (data.formStartId) {
                    ...
                    criteriaData.push(
                      "$('[name=" + data.element + "start]').val() + '_To_'  + "
                    )
                  }
                  if (data.formEndId) {
                    criteriaData.push(
...
TempalteEngine.prototype.toHtml = function (conf) {
  ...
  // html起始部分
  htmlCode.push('<section id="widget-grid">\n<!-- row -->\n<div class="row">\n')
