import { getCookie, getURLParameter } from './util.js'
import { currCtx } from './appFunc.js'
import { ajax, requireJs } from './ajax.js'
import politeCheck from './politeCheck.js'
import { subscribe } from './broadcaster.js'

// 是否使用DEMO資料
let _IS_USE_DEMO = true

//監控demo假資料，換資料的頻率
const _DEMO_DATA_LOOP_FREQ = 500
...
//判斷監控是否是使用Demo資料
function isDemoMonitor() {
  return _IS_USE_DEMO
}

function monitor(configObj) {
  var preAppId = getURLParameter('preAppId')
  var prePage = getURLParameter('prePage')
  var cncBrand = getURLParameter('cncBrand')
                  ...
                  success(data) {
                    ...
                    if (data['Storage'][configObj.machineId]) {
                      ...
                      if (configObj.customParamObj) {
                        ...
                        isGetDataSuccess = configObj.customCallback(newBoxData)
                      }
                    }
                  },
                  always() {
                    ...
                    if (!isGetDataSuccess && now < timeoutLimit) {
                      ...
                      if (isGetDataSuccess) {
                        ...
                        if (configObj.timeoutCallback) {
                          ...
                          if (configObj.customParamObj) {
                            //*** 增加一個可以傳參數給customCallback
                            configObj.timeoutCallback(configObj.customParamObj)
                          } else {
                            configObj.timeoutCallback()
                          }
                        } else {
          ...
          subscribe('Storage', {
            ...
            handler(data) {
              ...
              if (data[configObj.machineId]) {
                ...
                console.info(configObj.machineId + " 'Storage' data is empty!")
              }
            },
          })
        },
  ...
  function buildData(newBoxData, monitorParams, customCallback) {
    ...
    function scalar(monitorParam) {
      if (monitorParam.gCode) {
        ...
        if (!gCode) {
          console.warn('gCode is undefined, this ele id:' + monitorParam.ids[0])
          return
        }
        //有幾個系統就長幾個系統的資料
    ...
    function map(monitorParam, systemCount) {
      if (monitorParam.gCode) {
        ...
        if (gCode.type !== 'STRING') {
          console.warn(
            "monitor map must is 'STRING', this ele id: " + monitorParam.ids[0]
          )
          return
        } else {
          //有幾個系統就長幾個系統的資料
        ...
        console.warn("monitor map must has 'gCode' or 'gCodeKey, gCodeValue'.")
      }
...
/* 工具 */
monitor.tool = function (configObj) {
  execTool()
