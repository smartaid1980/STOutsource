      ...
      $.each(plot.getData(), function (ii, series) {
        ...
        if ((showMinValue || showMaxValue) && typeof (series.data[0]) != 'undefined') {
          ...
          for (var j = 1; j < series.data.length; ++j) {
            series.data[j][0] = +series.data[j][0];
            series.data[j][1] = +series.data[j][1];
            if (+series.data[j][0] < xMin) xMin = +series.data[j][0];
            if (+series.data[j][0] > xMax) xMax = +series.data[j][0];
            if (+series.data[j][1] < yMin) yMin = +series.data[j][1];
            if (+series.data[j][1] > yMax) yMax = +series.data[j][1];
          }
        } else {
          showMinValue = false;
          showMaxValue = false;
          for (var k = 0; k < series.data.length; ++k) {
            series.data[k][0] = +series.data[k][0];
            series.data[k][1] = +series.data[k][1];
          }
        }
