  ...
  GoGoAppFun({
    gogo: function (context) {
      context.reportTable = createReportTable({
        ...
        ajax: function (drawInfo, callback, table) {
          // 等到使用者觸發才會去畫欄位，此方法已脫離reporttable的機制(reporttable的callback會沒作用)
          const {
            start: startIndex,
            length: recordsAtOnce,
            order,
            draw: drawTimes,
          } = drawInfo
          const endIndex = startIndex + recordsAtOnce
          const orderColumnIndex = order[0].column
          const isAscending = order[0].dir === 'asc'
          const recordsTotal = context.tableObj.length
          let recordsFiltered
          var out = []
          var temp
          const searchInfo = table.aoPreSearchCols.map((obj) => {
            if (obj.bRegex) {
              return new RegExp(obj.sSearch)
            } else {
              return obj.sSearch
            }
          })
          // 篩選
          temp = context.tableObj.filter((rowData) => {
            return searchInfo.every((pattern, i) =>
              pattern instanceof RegExp
                ? pattern.test(rowData[i])
                : rowData[i].indexOf(pattern) >= 0
            )
          })
          ...
          temp.sort(
            (a, b) =>
              a[orderColumnIndex].localeCompare(b[orderColumnIndex]) *
              (isAscending ? 1 : -1)
          )
    ...
    util: {
      ...
      getData: function (startDate, endDate, machineList) {
        ...
        var calcProgress = function (percent) {
          setTimeout(function () {
          ...
          ctx
            ...
            .catch((reason) => {
              if (
                ...
                console.warn(reason, 'Go with demo.')
                    ...
                    (dataList, machineId) => {
                      return _.filter(dataList, (elem) => {
                        return (
                          ...
                          (ctx.min < elem.end_time &&
                            elem.end_time < ctx.max) ||
          ...
          _.each(data, function (dataList, machineId) {
            ...
            _.each(dataList, (elem) => {
              ...
              if (obj.rawTime > 0) {
                ...
                // machine_id, cnc_brand, status, alarm_code, macro_code, start_time, end_time, duration
                context.tableObj.push([
                  ...
                  obj.alarmCode == 'null' || !obj.alarmCode
                    ? ''
                    : `${obj.alarmCode} : ${obj.alarmCode}`,
                  obj.lightId == '12' &&
                  obj.macroCode !== '---' &&
                  obj.macroCode !== '0'
                    ? `${obj.macroCode} : ${context.getMacroProp(
                        obj.macroCode
                      )}`
                    : '',
                  obj.start,
          ...
          _.each(groupedPlotObj, function (plot, deviceId) {
            ...
            _.each(plot, function (elem) {
              ...
              let color =
                ...
                elem.macroCode !== '---' &&
                elem.macroCode !== '0'
                  ? ctx.getMacroProp(elem.macroCode, 'color_code')
