        ...
        setCoordinate2id(setting, htmlId) {
          ...
          this.getDeviceStatusVals(
            ...
            function (result) {
              ...
              //i18n('Monitoring_Distance_To_Go');
              var tableHtml =
                '<tr><td width="20%"></td><td class="head" width="20%">' +
                i18n('Monitoring_Absolute_Position') +
                '</td><td class="head" width="20%">' +
                i18n('Monitoring_Machine_Position') +
                '</td><td class="head" width="20%">' +
                i18n('Monitoring_Relative_Position') +
                '</td><td class="head" width="20%">' +
                i18n('Monitoring_Distance_To_Go') +
                '</td></tr>'
              if (result['G_SRNE()'] && result['G_SRNE()'][0]) {
                ...
                for (var index = 0; index < gSrneSize; index++) {
                  var gPosa = gPosaArr[index].replace('"', '')
                  var gPosm = gPosmArr[index].replace('"', '')
                  var gPosr = gPosrArr[index].replace('"', '')
                  var gPosd = gPosdArr[index].replace('"', '')
                  //數值就固定到小數點第三位
                  if (_.isFinite(gPosa)) {
                    gPosa = parseFloat(gPosa).toFixed(3)
                  }
                  if (_.isFinite(gPosm)) {
                    gPosm = parseFloat(gPosm).toFixed(3)
                  }
                  if (_.isFinite(gPosr)) {
                    gPosr = parseFloat(gPosr).toFixed(3)
                  }
                  if (_.isFinite(gPosd)) {
                    gPosd = parseFloat(gPosd).toFixed(3)
                  }
                  //多濾掉"，因為有時資料會多"
                  tableHtml =
                    tableHtml +
                    '<tr><td class="head">' +
                    gSrneArr[index].replace('"', '') +
                ...
                if (gSrneSize < defaultRowSize) {
                  for (var count = gSrneSize; count < defaultRowSize; count++) {
                    tableHtml =
                      tableHtml +
                      '<tr><td class="head">---</td><td class="value text-right">---</td><td class="value text-right">---</td><td class="value text-right">---</td><td class="value text-right">---</td></tr>'
                  }
                }
              } else {
                //沒有就使用空的table
                for (var j = 0; j < defaultRowSize; j++) {
                  tableHtml =
                    tableHtml +
