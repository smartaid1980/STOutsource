...
export default function () {
  GoGoAppFun({
    gogo: function () {
      ...
      ;(function () {
        $('#myTab a[href=' + window.sessionStorage.tab + ']').tab('show')
          ...
          servkit.subscribe('DeviceStatus', {
            ...
            handler: function (data) {
              ...
              _.each(demoLights, function (demoStatus, demoMachineId) {
                //console.warn(demoMachineId + ":" + demoStatus);
        ...
        function updateTableStatus($plantAreaTable, $buildSearchTable) {
          if (currentLights) {
            console.warn('update device status.')
            ...
            console.warn('Not find deviceStatus file!!')
          }
        }
        ...
        function allPlantIdCallback(data) {
          ...
          //清單一建完就建立清單上第一個廠區
          var plantArea
          if (window.sessionStorage.plantArea) {
            plantArea = window.sessionStorage.plantArea
            window.sessionStorage.removeItem('plantArea')
            $plantAreaForm.val(plantArea)
          } else {
            plantArea = $plantAreaForm.val()
          }
          buildPlantAreaTable(plantArea, plantAreaTableCallback)
          initSearchTalbe()
        }
        ...
        function plantAreaTableCallback(data) {
          ...
          if (data.area != undefined) {
            ...
            _.each(data.area, function (record, rowKey) {
              ...
              _.each(record, function (col, colKey) {
                ...
                if (col != '' && rowKey != 0 && colKey != 0) {
                  ...
                  $td.attr(
                    'title',
                    deviceMap[machineId] ? deviceMap[machineId] : machineId
                  )
                ...
                if (deviceMap[machineId] == undefined) {
                  if (machineId)
                    $td.append(
                      machineId.length > 10
                        ? '...' + machineId.substring(machineId.length - 7)
                        : machineId
                    )
                } else {
                  //將id換成name
                  if (machineId)
                    $td.append(
                      deviceMap[machineId].length > 10
                        ? '...' +
                            deviceMap[machineId].substring(
                              deviceMap[machineId].length - 7
                            )
                        : deviceMap[machineId]
                    )
                }
            ...
            // $plantAreaTable.append('<col style="background-color:' + headBgColor + '"/>'); //第一欄是row head要灰底
        ...
        function bindTarget2Monitor() {
          var url = '#app/EquipMonitor/function/' + getCookie('lang') + '/'

          if (servkit.configUseDeviceStatus()) {
            //現在分成使用deviceStatus監控和一般送命令給box的監控
            url = url + '01_info_by_device_status.html'
          } else {
            url = url + '01_info.html'
          }
          $('#myTabContent1')
            .off('click')
            .on('click', '.machineLight', function () {
              var tabTag = '#' + $(this).closest('.tab-pane').attr('id')
              window.sessionStorage.setItem('tab', tabTag)
              ...
              if (boxId != undefined) {
                ...
                if (
                  encodeCncBrand === undefined ||
                  encodeCncBrand.trim() === 'OTHER' ||
                  encodeCncBrand.trim() === 'INDICATORLAMP_FILE' ||
                  encodeCncBrand.trim() === 'INDICATORLAMP_ICP_TET_PD6'
                ) {
                  $.smallBox({
                    title:
                      `${i18n('Machine')} '` +
                ...
                if (encodeCncBrand.trim() === 'REALTIME') {
                  var lang = getCookie('lang')
                  var realtimeUrl =
                    '#app/EquipMonitor/function/' +
                    lang +
                    '/01_info_by_device_status_a_plus.html'
                ...
                console.warn('machine not bind box!')
              }
              window.sessionStorage.plantArea = $plantAreaForm.val()
            })
        }
