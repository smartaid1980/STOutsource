    ...
    gogo: function (context) {
      ;(function () {
        ...
        function createPlantAreaMatrix(plantId) {
          ...
          var noPlantMachinesArr = machineNameIdArray
          var noExistMachines = []
          ...
          $.each($('#buildPlantArea').find('tr'), function (rowKey) {
            ...
            $.each($(this).find('td'), function (columnKey) {
              ...
              if (rowKey === 0 || columnKey === 0) {
                ...
                deviceName = $(this).children('input').val()
        ...
        function buildTable(data) {
          var $table = $(
            '<table id="buildPlantArea" class="table-bordered" width="100%" style="table-layout:fixed;"><col style="background-color:#e8e8e8"/></table>'
          )
            ...
            $.each(this, function (colKey) {
              var $td = $(
                '<td id="row' +
                  ...
                  '-col' +
                  colKey +
                  '" style="border: 1px solid #cecece;"></td>'
              )
              if (colKey === 0 || rowKey === 0) {
                ...
                $td.append(
                  '<input type="text" class="useAutocomplete" list="productName" style="border:none;display:block;width:100%;" value="' +
          ...
          servkit.ajax(
            ...
            },
            {
              success: function (data) {
                ...
                machineNameIdArray = []
                _.each(data, function (ele) {
                  machineNameIdArray.push(servkit.getMachineName(ele.device_id))
                })
                $('.useAutocomplete').autocomplete({
                  source: machineNameIdArray,
                  minLength: 1,
                  open: function (evt) {
                    $('ul.ui-menu')
                      .innerWidth(evt.target.cssWidth)
                      .height('100px')
                      .css('overflow', 'auto')
                  },
                  search: function (oEvent) {
                    let search = []
                    var inputDataArr = _.map(
                      $('#buildPlantArea').find('input'),
                      function (data) {
                        return $(data).val()
                      }
                    )
                    _.each(machineNameIdArray, function (data) {
                      var chk = _.find(inputDataArr, function (inputData) {
                        if (data === inputData) return true
                      })
                      if (!chk) search.push(data)
                    })
      ...
      servkit.crudtable({
        ...
        tableModel: 'com.servtech.servcloud.module.model.Plant',
        create: {
