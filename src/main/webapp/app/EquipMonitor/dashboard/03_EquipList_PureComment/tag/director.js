function render(widget, done, machineMap) {
  ...
  var compiled_list = _.template(
    '<div id="<%=prefix%><%=id%>" class="<%=clazz%>" data-id="<%=id%>">' +
      ...
      '           <div class="big">{i18n_ServCloud_small_Product_ID}</div>' +
      '       </div>' +
      '       <div class="tab-panel txt-quantity">' +
      '           <div class="big">{i18n_ServCloud_Quantity}</div>' +
      '       </div>' +
      '   </div>' +
      '</div>' +
      '</div>'
  ...
  function _updateDeviceCard(el, data) {
    //{i18n_ServCloud_small_Product_ID}
    if (data.program)
      data.program = '{i18n_ServCloud_small_Product_ID}:' + data.program
    if (data.quantity == 0) data.quantity = '{i18n_ServCloud_Quantity}:0'
    else data.quantity = '{i18n_ServCloud_Quantity}:' + data.quantity
    ...
    el.removeClass('online idle alarm')
      .addClass(status)
      .find('.txt-program .big')
      .html(program || '')
      .end()
      .find('.txt-quantity .big')
      .html(quantity || '')
    _updateEasyPieChart(
  ...
  //       widget.title = "<i class='fa fa-bar-chart-o'></i> <strong>" + 'Machine List' + '-' + plantId + '</strong>'

  //       widget.asJquery().find('h2').replaceWith('<h2>' + widget.title + '</h2>')
  //     }
  //   })
  // }
  function worse_input(data) {
    var chk = ['-1', '-2', 'B']
    for (var i = 0; i < chk.length; i++) {
      if (data == chk[i]) return i
    }
    return 3
  }
  function check_PRCM(data, old) {
    var content
    if (worse_input(data) >= worse_input(old)) {
      content = data
    } else {
      content = old
    }
  ...
  if (!widget.parameter.disable) {
    ...
    _.map(machineMap, function (machineData, machineId) {
      ...
      if (machineId.startsWith('_')) {
        if (machineData['G_CONS()']) cons = machineData['G_CONS()'][0]
        if (machineData['G_ACTF()']) feed = toNumber(machineData['G_ACTF()'][0])
        if (machineData['G_SPMS()'])
          speed = toNumber(machineData['G_SPMS()'][0])
        if (machineData['G_SPSO()'])
          speed_rate = toNumber(machineData['G_SPSO()'][0])
        if (machineData['G_FERP()'])
          feed_rate = toNumber(machineData['G_FERP()'][0])
        if (machineData['G_RPCM()'] && machineData['G_PGCM()'])
          program = check_PRCM(
            machineData['G_RPCM()'][0],
            machineData['G_PGCM()'][0]
          )
        if (machineData['G_TOCP()'])
          quantity = toNumber(machineData['G_TOCP()'][0])
