    ...
    gogo: function (context) {
      ...
      $('#submit').on('click', function (e) {
        ...
        var selectedDevice = context.$stkReportForm
          .find("[name='device']")
          .val()
        // selectedDevice = _.filter(selectedDevice, (str) => str === 'All')
              ...
              success: function (shiftData) {
                ...
                hippoInstans.exhale(function (exhalable) {
                  ...
                  _.each(productConsumeTimeByWorkOrder, (obj) => {
                    if (
                      obj['M2'] === undefined &&
                      obj['Setting Time (M2)'] === undefined
                    ) {
                      obj['M2'] = 0.0
                    } else {
                      obj['M2'] = obj['Setting Time (M2)']
                    }
                    ...
                    achievementCount++
                    achievementTotal += obj.output_ratio
                    // if (obj.power_time != 0 && obj.total_time != 0 && obj.standard_work_time != 0 && obj.act_work_time != 0) {
                    //   achievementCount++
                    //   achievementTotal += obj.output_ratio
                    // } else {
                    //   delete obj.property
                  ...
                  const machineList = context.$stkReportForm
                    .find("[name='device']")
                    .val()
                  const selectSize = _.filter(machineList, (value) => {
                    return value !== 'ALL'
                  }).length
                  if (_.keys(productConsumeTime).length !== 0) {
                    $('#total-achievement-rate').text(
                      ((achievementTotal / achievementCount) * 100).toFixed(2) +
                        '%'
                    )
                    $('.jarviswidget:gt(3)').find('.fa-minus').click() //collapse last three widgets
                    productConsumeTime.off_time =
                      (totalShiftTime / 60000) * selectSize -
                      productConsumeTime.power_time
                    productConsumeTime.off_time =
      ...
      context.$workOrderTable.on('click', 'tr', function (e) {
        e.preventDefault()
        _.each($(this.parentElement).find('tr'), (row) => {
          $(row).css('background-color', 'white')
        })
        $(this).css('background-color', servkit.colors.blue)
        var rowData = workOrderTable.row(this).data()
        let deviceId = ''
        _.each(servkit.getMachineMap(), (val, key) => {
          if (val.device_name === rowData.device_id) {
            deviceId = key
          }
        })
        var key =
          deviceId + '@@' + rowData.product_id + '@@' + rowData.work_order_id
      ...
      // context.$stkReportForm.find("[name='startDate']").val('2018/11/01')
    ...
    util: {
      ...
      workOrderColumns: [
        ...
        },
        {
          ...
          tooltip: i18n('Total_Power_Time') + ' / ' + i18n('Part_Count'),
        },
        {
      ...
      init: function () {
        ...
        // servkit.initMachineSelect($("select[name=device]"), true);
          ...
          ],
          {
            ...
            tooltipOpts: {
              ...
              content: function (label, x, y) {
                let sss = x.toString()
                sss = sss.includes('.')
                  ? Math.floor(('0.' + sss.split('.')[1]) * 60)
                  : '0'
                return `${Math.floor(x)} M ${sss}S`
              },
            },
          }
        )
      ...
      equipmentExcel: function () {
        ...
        servkit.downloadCustomizedExcel($('.equipment-excel-btn'), function () {
          console.log('downloadCustomizedExcel')
          return {
            templateName: 'empty',
            fileName:
                ...
                data: [
                  [
                    $('[name=startDate]').val() +
                      ' - ' +
                      $('[name=endDate]').val(),
                    servkit.getMachineNameStringSplitByComma(
                      $('[name=device]').val()
                    ),
                  ],
                ],
