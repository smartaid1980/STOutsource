...
export default async function () {
  ...
  GoGoAppFun({
    ...
    util: {
      ...
      main() {
        ...
        $('#year')
          .on('dp.change', function () {
            var hideAfter = false
            if (this.value === String(new Date().getFullYear()))
              hideAfter = true
        ...
        // servkit.initSelectWithList(processMap, $('#op'))
        var demoData = {
          quarter: [
            {
              timeSeries: '1',
        ...
        $('#query-btn').on('click', function (evt) {
          ...
          servkit.ajax(
            {
              url: 'api/yihcheng/kpi-view-table/fourTable',
              type: 'POST',
      ...
      update(type) {
        this[type].target = this.targetValue
        this[type].drawChart()
      },
      getDataMap(data) {
        var dataMap = {}
        _.each(
          ...
          (val) =>
            (dataMap[val.timeSeries] = {
              value: (Number(val.goQuantity) / Number(val.output)) * 100,
              primary: val,
            })
        )
        return dataMap
      },
      getStartDate(dataList) {
