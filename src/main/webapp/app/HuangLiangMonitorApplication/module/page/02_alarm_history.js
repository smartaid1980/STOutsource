...
export default function () {
  ...
  ;(function () {
    var monitor = function () {
      var monitorCmd = {
        //box監控命令
        name: 'cnc_AlarmHistory',
        cycleType: 'count',
        cycleValue: 1,
        timeout: 0,
        items: [
          { signal: { id: 'G_AAMH' }, collect: { waitMs: 0, count: 1 } }, //機械座標
        ],
      }

      var tableHeadColumns = [
        { id: 'time', name: `${i18n('Time')}` },
        { id: 'alarmCode', name: `${i18n('Alarm_Code')}` },
        { id: 'alarmCodeDesc', name: `${i18n('Alarm_Description')}` },
      ]
      ...
      servkit.monitor({
        ...
        customCallback: function (data) {
          ...
          if (data['G_AAMH()']) {
            //data = {"G_AAMH()":{"type":"STRING","values":[["2015-10-07 20:09:05|-1"]]}};

            var alarmHistory = data['G_AAMH()']
            //console.log(alarmHistory);
            var times = [],
              alarmCodes = []
            _.each(alarmHistory.values[0], function (ele) {
              var array = ele.split('|')
              times.push(array[0])
              alarmCodes.push(array[1])
            })
            servkit.monitor.tool({
              ...
              config: {
                ...
                callback: function (alarmCodeMap) {
                  var matrix = []
                  _.each(alarmCodes, function (ele, index) {
                    matrix.push([times[index], ele, alarmCodeMap[ele]])
                  })
