...
export default function () {
  ...
  ;(function () {
    ...
    function initSearchTalbeCallback(data) {
      ...
      _.each(data.machines, function (ele) {
        ...
        if (currentLights[ele.device_id] != undefined) {
          statusColor =
            machineLightObj[currentLights[ele.device_id][machineStatusIndex]]
              .color
        } else {
    ...
    function bindTarget2Monitor() {
      ...
      console.log(lang)
      var url =
        '#app/HuangLiangMonitorApplication/function/' + lang + '/01_info.html'
      $('.machineLight')
        .off('click')
        .on('click', function () {
          var boxId
