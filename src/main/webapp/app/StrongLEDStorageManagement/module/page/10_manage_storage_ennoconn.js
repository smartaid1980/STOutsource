...
function fetchStoreType() {
  return new Promise((res) =>
    ajax(
      ...
      {
        success(data) {
          res(_.indexBy(data, 'store_type_id'))
        },
      }
    )
  )
...
export default async function render() {
  ...
  const storeTypeData = await fetchStoreType()
  const storeTypeOptionList = Object.entries(storeTypeData).map(
    ([store_type_id, { store_type_name }]) =>
      `<option style="padding: 3px 0px 3px 3px" value="${store_type_id}">${store_type_name}</option>`
  )
  ...
  GoGoAppFun({
    ...
    util: {
      ...
      addNodeFormGroup() {
        ...
        const isStoreParentLevel =
          Number(context.levelOfCurrNode) === context.levelCount - 2
        ...
        <section class="col col-xs-12 col-sm-12 col-md-4 col-lg-4">
          <label class="form-text">${i18n('ID')}</label>
          <label class="input">
        ...
        <section class="col col-xs-12 col-sm-12 col-md-4 col-lg-4">
          <label class="form-text">${i18n('Name')}</label>
          <label class="input">
            <input class="form-control" data-type="name" type="text">
          </label>
        </section>
        <section class="col col-xs-12 col-sm-12 col-md-3 col-lg-3 ${
          isStoreParentLevel ? '' : 'hide'
        }">
          <label class="form-text">${i18n('Shelf_Type')}</label>
          <label class="select">
            <select class="form-control" data-type="type" type="text">${storeTypeOptionList.join(
              ''
            )}</select>
            <i></i>
          </label>
        </section>
        <section class="col col-xs-12 col-sm-12 col-md-1 col-lg-1" style="display: flex; justify-content: center; align-items: flex-end;">
      ...
      addNode() {
        ...
        const idPrefix = storageConfig.positionTag || ''

        context.loadingAddBtn.doing()

        for (const el of articleList) {
          const $id = $(el).find('[data-type=id]')
          const $name = $(el).find('[data-type=name]')
          const $type = $(el).find('[data-type=type]')
          const name = $name.val()
          const id = $id.val()
          const type = $type.val()
          const nodeData = {
          ...
          // 新增 store 順便根據料架類型新增底下的 position
          if (isStoreParentLevel) {
            const positionCount = storeTypeData[type].store_type_cell
            const lastLightId = context.getLastLightId()
            let id

            nodeData.store_type_id = Number(type)
            _.times(positionCount, (i) => {
              id = (i + 1).toString()
              nodeData.child.push({
                id,
                name:
                  idPrefix +
                  id.padStart(storageConfig.positionLength || 1, '0'),
                level: String(context.levelOfCurrNode + 2),
                type: '',
                child: [],
                light_id: (lastLightId + 1 + i).toString(),
              })
            })
          }
        ...
        context.treeView = new TreeView(
          ...
          $treeViewContainer,
          {
            afterTreeDraw(treeContainer, tree) {
              ...
              const nodeList = Array.from(firstUlEl.children)
            ...
            afterNodeDraw(container, index, node) {
              const isStoreLevel =
                Number(node.level) === this.root.levelCount - 1
              const span = container.firstElementChild
              const { hasChildren, data, level } = node
              const { id, name, db_id, store_type_id } = data || {}
              const { levelCount } = this.root
              // 判斷 store_type_id 不是 null 或 undefined
              const store_type_name =
                isStoreLevel && Number.isInteger(store_type_id)
                  ? storeTypeData[store_type_id].store_type_name || ''
                  : ''
              ...
              if (hasChildren && level < levelCount - 1) {
                // 因為隱藏最後一層，檢查是 store 層以上
      ...
      getNodeInfo($ele, isCreate) {
        ...
        context.currNodeIndexPath.forEach((nodeIndex, index) => {
          currNodeData = currNodeListData[nodeIndex]
          if (index + 1 !== context.currNodeIndexPath.length) {
            currNodeListData = currNodeData.child
          }
        })
        context.currNodeData = currNodeData
        context.currNodeListData = currNodeListData
