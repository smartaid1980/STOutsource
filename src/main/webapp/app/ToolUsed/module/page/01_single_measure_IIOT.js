  ...
  GoGoAppFun({
    ...
    util: {
      ...
      detail: function (dayTable, detailTable) {
        ...
        hippo
          .newMashupExhaler()
          .space('tool_part_detail:tool')
          .index('machine_id', machineList)
          .indexRange('date', startDate, endDate)

          .space('tool_part_detail_pure_comment:comment')
          .index('machine_id', machineList)
          .indexRange('date', startDate, endDate)

          .column('tool', 'start_time')
          .column('tool', 'end_time')
          .column('tool', 'machine_id')
          .column('tool', 'date')
          .column('tool', 'work_shift')
          .column('tool', 'program_name')
          .column('tool', 'tool')
          .column('tool', 'power_millisecond')
          .column('tool', 'operate_millisecond')
          .column('tool', 'cutting_millisecond')
          .column('tool', 'idle_millisecond')
          .column('tool', 'alarm_millisecond')
          .column('tool', 'work_shift_millisecond')

          .column('comment', 'pure_comment')

          .mashupKey('machine_id', 'date', 'program_name', 'tool')
          .exhale(function (exhalable) {
            try {
              var result = exhalable.map(function (data) {
                var timeData = that.commons.millisecondExcludMillisecond(
                  data.tool[0]
                )

                return [
                  that.getDBObject[data.tool[0].machine_id],
                  timeData.date,
                  timeData.work_shift,
                  data.comment.length > 0
                    ? data.comment[0].pure_comment
                    : 'N.A.',
                  timeData.tool,
                  timeData.start_time.dateTimeBitsToFormatted(),
                  timeData.end_time.dateTimeBitsToFormatted(),
                  ...
                  (
                    timeData.idle_millisecond + data.alarm_millisecond
                  ).millisecondToHHmmss(),
                ]
              })
      ...
      day: function (dayTable, workShiftTable) {
        ...
        hippo
          .newMashupExhaler()
          .space('tool_part_day:tool')
          .index('machine_id', machineList)
          .indexRange('date', startDate, endDate)

          .space('tool_part_day_pure_comment:comment')
          .index('machine_id', machineList)
          .indexRange('date', startDate, endDate)
