  ...
  GoGoAppFun({
    gogo: function gogo(ctx) {
      ...
      $('#submit-btn').on('click', function (evt) {
        ...
        //入庫單號select
        let bill_no = $('#bill_no_select').val()
        ...
        //餘料入庫 radio
        let residue = $('[name=residue_warehousing]:checked').val()
        residue = residue == '11' ? `AND situation ='11'` : `AND situation='12'`
        ...
        servkit.ajax(
          {
            ...
            data: JSON.stringify({
              ...
              whereClause: `${date} ${warehouse} ${bill_no} ${pkgid} ${residue} ${transferred_out}`,
            }),
          },
          {
            success: function (data) {
              ...
              _.each(data, (val) => {
                let arr = [
                  ...
                  val.vender_lot || '---',
                  val.ware_id || '---',
                  (val.situation = val.situation === '11' ? '否' : '是'),
      ...
      servkit.downloadExcel($('#no_export'), () => {
        if (ctx.ReportTable.getSelectedRow().length !== 0) {
          ...
          let matrix = _.map(ctx.ReportTable.getSelectedRow(), (val) => {
            return val.filter((i, index) => index < 10)
          })
          return {
            ...
            },
            {
              success: function () {
                _.each(ctx.ReportTable.getSelectedTrArray(), (val) => {
                  val.cells[10].innerHTML = '是'
                  val.cells[10].innerTEXT = '是'
                })
              },
            }
          )
        ...
        servkit.ajax(
          ...
          },
          {
            success(data) {
              ...
              //入庫
              let bill_no = data.map((i) => i.bill_no).filter((i) => i)
              let bill_no_arr = _.uniq(bill_no)
              bill_no_arr.unshift('')

              bill_no = bill_no.length === 0 ? ['目前無入庫單資料'] : bill_no
              servkit.initSelectWithList(bill_no_arr, $('#bill_no_select'))
              $('#bill_no_select').select2({
                minimumInputLength: 0,
                allowClear: true,
                placeholder: '選擇入庫單號',
              })
              select2AllowClearHelper($('#bill_no_select'))
