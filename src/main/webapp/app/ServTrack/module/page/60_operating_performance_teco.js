  ...
  GoGoAppFun({
    gogo: function (ctx) {
      ...
      ctx.$demoBtn.on('click', function (evt) {
        ...
        ctx.$staffId.val(showdemoConfig.product).trigger('change.select2') // only for select2 plugin
        if (showdemoConfig.product === '') {
          $('#s2id_staffId .select2-chosen').text('')
        }
    ...
    util: {
      ...
      $staffNameTitle: $('#staff-name-title'),
      ...
      drawTable: function (dayTable) {
        ...
        var params = {
          ...
          staffId: that.commons.checkEscapeSymbol(that.$staffId.val()),
        ...
        var staffName =
          that.$staffId.val() == null
            ? ''
            : that.staffFunc.getName(that.$staffId.val())
        that.$staffNameTitle.html(staffName)

        servkit.ajax(
          {
            url:
              servkit.rootPath +
              '/api/teco/servtrack/operatingperformance/readperformance',
            type: 'POST',
            ...
            success: function (dbData) {
              _.map(dbData, function (data) {
                ...
                subArray.push(
                  data.cust_field_1 == -1
                    ? '---'
                    : that.staffFunc.getName(data.cust_field_1)
                )
                subArray.push(
        ...
        servkit.ajax(
          {
            ...
            contentType: 'application/json',
            data: JSON.stringify({
              table: 'a_teco_servtrack_staff',
              columns: ['staff_id', 'staff_name', 'is_open'],
