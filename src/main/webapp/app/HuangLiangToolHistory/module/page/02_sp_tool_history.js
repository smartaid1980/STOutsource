    ...
    util: {
      ...
      getToolSpHistory() {
        ...
        if (
          ...
          whereParams.push(queryParam.mstock_name[0])
        }
        ...
        if (Object.prototype.hasOwnProperty.call(queryParam, 'sample_id')) {
          whereClause += ' AND sample_id = ?'
          whereParams.push(queryParam.sample_id)
        }
        if (Object.prototype.hasOwnProperty.call(queryParam, 'sample_pid')) {
          whereClause += ' AND sample_pid = ?'
          whereParams.push(queryParam.sample_pid)
        }
        if (
      ...
      initToolHistoryInfoForm() {
        ...
        servkit.initSelectWithList(
          context.processList,
          $(toolHistoryInfoForm.elements.process.element)
        )
        servkit.initSelectWithList(
          Object.values(context.mstockNameMap),
          $(toolHistoryInfoForm.elements.mstock_name.element)
        )
        servkit.initSelectWithList(
      ...
      updateToolHistory() {
        ...
        const tool_history_no = $toolHistoryModal.data('rowData')
          .tool_history_no
        const toolSpHistory = _.mapObject(
        ...
        // TODO: table不能換頁
        const toolSpHisList = toolHistoryListTable.table.data().length
        ...
        servkit.ajax(
          {
            url: ' api/huangliangTool/toolhistory/updateToolSpHistory',
            type: 'PUT',
            contentType: 'application/json',
            data: JSON.stringify({
              toolSpHistory,
