...
/**
 * Created by Kevin on 2020/8/10.
 */
@RestController
@RequestMapping("/yihcheng/kpi-view-table")
public class YihChengKpiViewTableController {

    private static final Logger logger = LoggerFactory.getLogger(YihChengKpiViewTableController.class);
    ...
    public RequestResult<?> getNGQuality(@RequestBody final Map data) {
        return ActiveJdbc.operTx(new Operation<RequestResult<?>>() {
            ...
            public RequestResult<?> operate() {
                ...
                List<String> productSetList = new ArrayList<>();
                ...
                List<WorkTrackingNGQualityView> resultDB = WorkTrackingNGQualityView.find("shift_day >=? and shift_day <= ?" +
                                " and process_code in " + processCodeSB.toString() +
                                ignoreProductIdSqlIfNull(" and product_id in ", productIdsSB) +
                                " and division_id in " + divisionIdsSB.toString()
                        , queryStart, queryEnd);
                ...
                for (WorkTrackingNGQualityView wt : resultDB) {
                    ...
                    String product_set = wt.getString("product_set") == null ? "" : wt.getString("product_set");
                    String line_name = wt.getString("line_name");

                    if (!ngNameList.contains(ng_name)) {
                        ngNameList.add(ng_name);
                    }
                    if (!productSetList.contains(product_set)) {
                        productSetList.add(product_set);
                    }
                ...
                int OutputArray[][][][][] = new int[processCodesList.size() + 1][ngNameList.size() + 1][productSetList.size() + 1][lineNameList.size() + 1][3];
                int NGQuantityArray[][][][][] = new int[processCodesList.size() + 1][ngNameList.size() + 1][productSetList.size() + 1][lineNameList.size() + 1][3];
                    ...
                    if (output != 0) {
                        ...
                        for (int i = 0; i < processCodesList.size(); i++) {
                            for (int j = 0; j < ngNameList.size(); j++) {
                                for (int k = 0; k < productSetList.size(); k++) {
                                    for (int m = 0; m < lineNameList.size(); m++) {
                                        if (processCodesList.get(i).equals(process_code) && ngNameList.get(j).equals(ng_name)
                                                && productSetList.get(k).equals(product_set) && lineNameList.get(m).equals(line_name)) {
                        ...
                        if (NGQuantityArray[i][j + 1][0][0][0] != 0) {
                            ...
                            for (int k = 0; k < productSetList.size(); k++) {
                                if (OutputArray[i][0][k + 1][0][0] != 0) {
                                    ...
                                    resultProducts.put("productName", productSetList.get(k));
    ...
    public RequestResult<?> kpiViewTableWeek(@RequestBody final Map data) {
        return ActiveJdbc.operTx(new Operation<RequestResult<?>>() {
            ...
            public RequestResult<?> operate() {
                ...
                result.put("quarter", getTrackingQuarter(year, processCodesList, productIdList, divisionIdList));
                result.put("month", getTrackingMonth(year, processCodesList, productIdList, divisionIdList));
                result.put("week", getTrackingWeek(year, month, processCodesList, productIdList, divisionIdList));
                result.put("day", getTrackingDay(year, month, processCodesList, productIdList, divisionIdList));
    ...
    //取得日報表資料
    public List<Map<String, Object>> getTrackingDay(String queryYear, String queryMonth, List<String> processCodes, List<String> productIdList, List<String> divisionIdList) {
        StringBuilder processCodeSB = ListToSQLStr(processCodes);
        StringBuilder productIdsSB = ListToSQLStr(productIdList);
        StringBuilder divisionIdsSB = ListToSQLStr(divisionIdList);
        ...
        List<WorkTrackingKpiView> wt = WorkTrackingKpiView.find("shift_day >=? and shift_day <= ?" +
                        " and process_code IN " + processCodeSB.toString() +
                        ignoreProductIdSqlIfNull(" and product_id in ", productIdsSB) +
                        " and division_id IN " + divisionIdsSB.toString()
                , queryStart, queryEnd);
    ...
    //取得周報表資料
    private List<Map<String, Object>> getTrackingWeek(String year, String month, List<String> processCodes, List<String> productIdList, List<String> divisionIdList) {
        StringBuilder processCodeSB = ListToSQLStr(processCodes);
        StringBuilder productIdsSB = ListToSQLStr(productIdList);
        StringBuilder divisionIdsSB = ListToSQLStr(divisionIdList);
...
//                    }
    ...
    public List<Map<String, Object>> getTrackingMonth(String queryYear, List<String> processCodes, List<String> productIdList, List<String> divisionIdList) {
        StringBuilder processCodeSB = ListToSQLStr(processCodes);
        StringBuilder productIdsSB = ListToSQLStr(productIdList);
        StringBuilder divisionIdsSB = ListToSQLStr(divisionIdList);
    ...
    public List<Map<String, Object>> getTrackingQuarter(String queryYear, List<String> processCodes, List<String> productIdList, List<String> divisionIdList) {
        StringBuilder processCodeSB = ListToSQLStr(processCodes);
        StringBuilder productIdsSB = ListToSQLStr(productIdList);
        StringBuilder divisionIdsSB = ListToSQLStr(divisionIdList);
        ...
        return ActiveJdbc.operTx(new Operation<RequestResult<?>>() {
            ...
            public RequestResult<?> operate() {
                ...
                try {
                    ...
                    List<Map> TILViews = getQueryYearViewDataByJava(year, processCodesList, productIdList, divisionIdList);
    ...
    private List<Map> getQueryYearViewDataByJava(String year, List<String> processCodesList, List<String> productIdList, List<String> divisionIdList) throws ParseException {
        long time0 = System.currentTimeMillis();
        List<Map> workTrackingList = Base.findAll(getWorkTrackingSql(year, processCodesList, productIdList, divisionIdList));
...
//            trackingEfficiencyView.fromMap(workTracking);
    ...
    private String getWorkTrackingSql(String year, List<String> processCodes, List<String> productIdList, List<String> divisionIdList) throws ParseException {
        StringBuilder processCodeSB = ListToSQLStr(processCodes);
        StringBuilder productIdsSB = ListToSQLStr(productIdList);
        StringBuilder divisionIdsSB = ListToSQLStr(divisionIdList);
        ...
        String sql = "SELECT a.* FROM (SELECT ROUND((wt.go_quantity * vwo.std_hour/60),4) AS std_workhour, vwo.product_id, vwo.product_name, vwo.division_id, vwo.product_set, vwo.process_name, vwo.process_code, l.line_name, wt.* FROM " + whereSql + " wt " +
                ...
                "where a.process_code in " + processCodeSB.toString() +
                ignoreProductIdSqlIfNull(" and a.product_id in ", productIdsSB) +
                " and a.division_id in " + divisionIdsSB.toString();
        System.out.println("sql : " + sql);
        return sql;
    }

    private String ignoreProductIdSqlIfNull(String s, StringBuilder productIdsSB) {
        if (productIdsSB == null)
            return "";
        return s + productIdsSB.toString();
    }
