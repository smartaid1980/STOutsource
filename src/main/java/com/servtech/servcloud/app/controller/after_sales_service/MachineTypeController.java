...
import com.servtech.servcloud.app.model.after_sales_service.MachineType;
 ...
 */
@RestController
@RequestMapping("/aftersalesservice/machinetype")
public class MachineTypeController {
    private final Logger logger = LoggerFactory.getLogger(MachineTypeController.class);
        ...
        try {
            return ActiveJdbc.operTx(new Operation<RequestResult<String>>() {
                ...
                public RequestResult<String> operate() {
                    ...
                    machineType.fromMap(data);
                    if (machineType.insert()) {
                        return success(machineType.getString("machine_type_id"));
                    } else {
    ...
    public RequestResult<List<Map>> read() {
        return ActiveJdbc.oper(new Operation<RequestResult<List<Map>>>() {
            ...
            public RequestResult<List<Map>> operate() {
                return success(MachineType.findAll().include(Product.class).toMaps());
            }
        });
    }
    ...
    public RequestResult<Void> delete(@RequestBody final Object[] idList) {
        return ActiveJdbc.operTx(new Operation<RequestResult<Void>>() {
            ...
            public RequestResult<Void> operate() {
                int deleteAmount = MachineType.delete("machine_type_id IN (" + Util.strSplitBy("?", ",", idList.length) + ")", idList);
                return success();
            }
        });
    }
    ...
    public RequestResult<Void> excel()throws IOException {
        ...
        return ActiveJdbc.operTx(new Operation<RequestResult<Void>>() {
            ...
            public RequestResult<Void> operate() {
                ...
                String headerValue = "attachment; filename=\"MachineType-"+excelDate+".xlsx\"";
                response.setContentType(mimeType);
                response.setHeader(headerKey, headerValue);
                XSSFWorkbook workbook = new XSSFWorkbook();
                Sheet sheet = workbook.createSheet();
                Row row0  = sheet.createRow(0);
                row0.createCell(0).setCellValue("機種");
                row0.createCell(1).setCellValue("產品別代碼");
                row0.createCell(2).setCellValue("機種名稱");
                row0.createCell(3).setCellValue("機種說明");

                List<MachineType> list = MachineType.findAll();
                try {
                    int i = 1;
                    for (MachineType machineType : list) {
                        Row row = sheet.createRow(i);
                        row.createCell(0).setCellValue(machineType.get("machine_type_id").toString());
                        row.createCell(1).setCellValue(machineType.get("product_id").toString());
                        row.createCell(2).setCellValue(machineType.get("type_name").toString());
                        row.createCell(3).setCellValue(machineType.get("type_desc").toString());
                        i++;
                    }
