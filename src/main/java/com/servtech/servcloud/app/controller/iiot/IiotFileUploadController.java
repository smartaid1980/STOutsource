...
@RestController
@RequestMapping("/iiot/fileupload")
public class IiotFileUploadController {
    private static final Logger log = LoggerFactory.getLogger(IiotFileUploadController.class);
    private static final String FOLDER_NAME = "iiot_data_import";
    @Autowired
    ...
    public RequestResult<?> upload(@RequestParam("file") MultipartFile uploadFile,
        ...
        String batTarget = System.getProperty(SysPropKey.ROOT_PATH) + "/app/IIOTToolManagement/program/DataImport";

        File targetFolder = new File(System.getProperty(SysPropKey.DATA_PATH),
                FOLDER_NAME + "/" + dateStr.substring(0, 4) + "/" + dateStr.substring(4, 6) + "/" + excelName.replaceAll(".xlsx", ""));
        File targetFile = new File(targetFolder, dateStr + ".xlsx");
        try {
