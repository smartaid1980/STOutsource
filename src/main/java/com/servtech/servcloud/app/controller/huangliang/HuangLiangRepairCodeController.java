 ...
 */
@RestController
@RequestMapping("/repairCode")
public class HuangLiangRepairCodeController {
    ...
    try {
      return ActiveJdbc.operTx(new Operation<RequestResult<?>>() {
        ...
        public RequestResult<?> operate() {
          ...
          repairCode.fromMap(data);
          if (repairCode.insert()) {
            return success(data.get("repair_code").toString());
          } else {
            return fail("create repairCode fail...");
          }
        }
      });
    } catch (Exception e) {
  ...
  public RequestResult<?> update(@RequestBody final Map data) {
    return ActiveJdbc.operTx(new Operation<RequestResult<?>>() {
      ...
      public RequestResult<?> operate() {
        ...
        RepairCode repairCode = new RepairCode();
        repairCode.fromMap(data);
        if (repairCode.saveIt()) {
          return success(data.get("repair_code").toString());
        } else {
          return fail("update repairCode fail...");
        }
      }
    });
  }
  ...
  public RequestResult<List<Map>> read() {
    return ActiveJdbc.oper(new Operation<RequestResult<List<Map>>>() {
      ...
      public RequestResult<List<Map>> operate() {
        List<Map> repairCodes = RepairCode.findAll().toMaps();
        return success(repairCodes);
      }
    });
  }
  ...
  public RequestResult<String> delete(@RequestBody final Object[] idList) {
    return ActiveJdbc.operTx(new Operation<RequestResult<String>>() {
      ...
      public RequestResult<String> operate() {
        try{
          RepairCode.delete("repair_code IN (" + Util.strSplitBy("?", ",", idList.length) + ")", idList);
          return success();
        } catch (Exception e){
